      ...
      <>
        <div className={`border border-border rounded-xl ${className}`}>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle">
            <span className="text-[10px] font-bold font-mono uppercase tracking-widest text-txt-tertiary">
              Founder Profile
            </span>
            <button onClick={handleRefresh} className="p-1 text-txt-disabled hover:text-txt-secondary transition-colors" aria-label="프로필 새로고침">
              <RefreshCw size={10} />
            </button>
          </div>
          ...
          <div className="p-4">
            {founderType ? (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center text-lg bg-surface-sunken"
                  >
                    {founderType.emoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-sm text-txt-primary truncate">{founderType.name}</div>
                    <div className="text-[10px] text-txt-tertiary truncate">{founderType.taglineKo}</div>
                  </div>
                  <ChevronRight size={14} className="text-txt-disabled group-hover:text-txt-secondary transition-colors" />
                </div>
              </button>
            ) : profile ? (
              <div className="mb-3">
                <div className="font-bold text-sm text-txt-primary mb-2">{profile.founderType}</div>
                <div className="flex flex-wrap gap-1">
                  {profile.matchingTags.slice(0, 3).map((tag, idx) => (
                    <span key={idx} className="text-[9px] text-txt-tertiary font-mono">#{tag}</span>
                  ))}
                </div>
              </div>
            ) : (
              <div className="text-xs text-txt-disabled py-2">
                더 많은 결정을 내리면 분석됩니다
              </div>
            )}
            ...
            <div className="space-y-3 mt-4">
              {(['speedVsQuality', 'techVsBusiness'] as const).map(axis => (
                <div key={axis}>
                  <div className="flex justify-between text-[9px] text-txt-tertiary mb-1">
                    <span>{axisLabels[axis][0]}</span>
                    <span>{axisLabels[axis][1]}</span>
                  </div>
                  <div className="h-1 bg-surface-sunken rounded-full relative">
                    <div
                      className="absolute top-1/2 -translate-y-1/2 w-2 h-2 bg-surface-inverse rounded-full transition-all"
          ...
          {/* Footer Stats */}
          <div className="px-4 py-2.5 border-t border-border-subtle bg-surface-sunken">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 overflow-y-auto">
            <div className="relative">
              <button
                ...
                className="absolute -top-3 -right-3 z-10 w-7 h-7 bg-surface-card rounded-full shadow-md flex items-center justify-center text-txt-tertiary hover:text-txt-primary transition-colors border border-border"
                aria-label="닫기"
              >
  ...
  return (
    <div className={`border border-border rounded-xl overflow-hidden ${className}`}>
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border-subtle">
        <h3 className="font-bold text-sm text-txt-primary">창업자 성향 분석</h3>
        <button onClick={handleRefresh} className="p-1.5 text-txt-disabled hover:text-txt-secondary transition-colors" aria-label="프로필 새로고침">
          <RefreshCw size={12} />
        </button>
      </div>
      ...
      <div className="p-5 space-y-6">
        ...
        {profile && (
          <div>
            <div className="font-bold text-txt-primary mb-2">{profile.founderType}</div>
            <div className="flex flex-wrap gap-2">
              {profile.matchingTags.map((tag, idx) => (
                <span key={idx} className="text-[10px] text-txt-tertiary font-mono">
                  #{tag}
                </span>
              ))}
            </div>
        ...
        {/* 5-Axis Chart */}
        <div>
          <div className="text-[10px] font-bold font-mono uppercase tracking-widest text-txt-tertiary mb-4">
            5축 성향
          </div>
          <div className="space-y-4">
            {(Object.keys(axisLabels) as (keyof AxisScores)[]).map(axis => (
              <div key={axis}>
                <div className="flex justify-between text-[10px] text-txt-tertiary mb-1.5">
                  <span>{axisLabels[axis][0]}</span>
                  <span>{axisLabels[axis][1]}</span>
                </div>
                <div className="h-1.5 bg-surface-sunken rounded-full relative">
                  <div
                    className="absolute top-1/2 -translate-y-1/2 w-2.5 h-2.5 bg-surface-inverse rounded-full transition-all"
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            ...
            <div className="text-[9px] text-txt-tertiary mt-1">평균 응답</div>
          </div>
          <div className="text-center">
            <div className="text-xl font-bold text-txt-primary">
              {Math.round(behaviorPattern.customInputRatio * 100)}%
            </div>
            <div className="text-[9px] text-txt-tertiary mt-1">직접 입력</div>
          </div>
          <div className="text-center">
            <div className="text-xl font-bold text-txt-primary">
              {behaviorPattern.totalDecisions}
            </div>
            <div className="text-[9px] text-txt-tertiary mt-1">총 결정</div>
          </div>
        </div>
        ...
        {profile && (
          <div className="grid grid-cols-2 gap-6">
            <div>
              <div className="text-[9px] font-bold font-mono text-status-success-text uppercase tracking-widest mb-3">
                강점
              </div>
              <div className="space-y-2">
                {profile.strengths.map((s, idx) => (
                  <div key={idx} className="flex items-start gap-2 text-xs text-txt-secondary">
                    <span className="text-status-success-text font-bold">+</span>
                    <span>{s}</span>
                  </div>
                ))}
              </div>
            ...
            <div>
              <div className="text-[9px] font-bold font-mono text-status-warning-text uppercase tracking-widest mb-3">
                보완점
              </div>
              <div className="space-y-2">
                {profile.blindSpots.map((b, idx) => (
                  <div key={idx} className="flex items-start gap-2 text-xs text-txt-secondary">
                    <span className="text-status-warning-text font-bold">!</span>
                    <span>{b}</span>
                  </div>
                ))}
              </div>
        ...
        {profile && (
          <div className="pt-4 border-t border-border-subtle">
            ...
            <div className="text-sm text-txt-secondary">{profile.idealCofounder}</div>
          </div>
        )}
      </div>
    </div>
