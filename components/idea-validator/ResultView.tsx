...
import { Download, Share2, Briefcase, FileText, Layout, Cpu, Paintbrush, DollarSign, Shield, MapPin, Clock, Globe, Copy, Check, Target, Server, Zap, BarChart, Users, Star, Award, ArrowRight, Sparkles, Calendar, Gift, ExternalLink, ChevronRight } from 'lucide-react';
...
const EventTeaserCard = memo(({ event }: { event: RecommendedEvent }) => (
  <div className="bg-white border border-gray-200 rounded-sm p-4 hover:shadow-md hover:border-gray-300 transition-all group cursor-pointer">
  ...
  if (loading) {
    return (
      <div className="animate-pulse space-y-3">
        {[1, 2, 3].map(i => (
          <div key={i} className="bg-gray-100 rounded-sm h-24" />
        ))}
      </div>
    );
  }
...
const ActionCard = memo(({ title, items, color, icon }: { title: string, items: string[], color: string, icon: React.ReactNode }) => (
...
const PrdView = memo(({ prd }: { prd: PrdStructure }) => (
  <div className="bg-white shadow-lg rounded-sm border border-gray-200 overflow-hidden mx-auto max-w-5xl">
    {/* Header */}
          ...
          <section>
             ...
             <div className="space-y-3">
                {prd.targetAudience.map((target, i) => (
                   <div key={i} className="flex items-center gap-3 p-3 bg-pink-50/50 rounded-sm border border-pink-100">
          ...
          <section>
             ...
             <div className="space-y-3">
                {prd.successMetrics.map((metric, i) => (
                   <div key={i} className="flex items-center gap-3 p-3 bg-emerald-50/50 rounded-sm border border-emerald-100">
       ...
       <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             {prd.coreFeatures.map((feature, i) => (
                <div key={i} className="bg-white border border-gray-200 p-5 rounded-sm hover:shadow-md transition-shadow">
       ...
       <section>
          ...
          <div className="flex flex-wrap gap-2">
             {prd.techStack.map((tech, i) => (
                <span key={i} className="px-3 py-1.5 bg-indigo-50 border border-indigo-100 text-indigo-700 text-sm font-semibold rounded-sm">
                   {tech}
                </span>
             ))}
          </div>
...
const JdView = memo(({ jd, onCopy, copied }: { jd: JdStructure, onCopy: (text: string) => void, copied: boolean }) => (
  <div className="bg-white shadow-lg rounded-sm border border-gray-200 overflow-hidden mx-auto max-w-5xl relative">
     {/* Decorative Header */}
     <div className="h-40 bg-gray-900 relative pattern-grid-lg">
        <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-transparent to-transparent opacity-90"></div>
        <div className="absolute -bottom-10 left-8 md:left-12 p-1.5 bg-white rounded-sm shadow-lg">
           <div className="w-20 h-20 bg-black rounded-sm flex items-center justify-center text-white font-black text-3xl tracking-tighter">
              D.
           </div>
        </div>
     </div>
     ...
     <div className="p-8 md:p-12 grid grid-cols-1 md:grid-cols-3 gap-12">
         ...
         <div className="space-y-8">
            <div className="bg-gray-50 rounded-sm p-6 border border-gray-200">
               ...
               <div className="flex flex-wrap gap-2">
                  {jd.preferred.map((skill, i) => (
                     <span key={i} className="px-3 py-1 bg-white border border-gray-200 rounded-sm text-xs font-medium text-gray-600 shadow-sm">
                        {skill}
                     </span>
                  ))}
               </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-sm p-6 border border-blue-100">
...
const ResultView: React.FC<ResultViewProps> = ({ conversationHistory, originalIdea, reflectedAdvice = [], onComplete }) => {
  ...
  return (
    <div className="w-full py-4 px-3 md:px-4">
      ...
      <div className="mb-8 bg-white border border-gray-200 rounded-sm p-6 md:p-8 flex flex-col md:flex-row items-center justify-between shadow-sm">
          <div className="flex items-center gap-6">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                 ...
                 <span className="px-2 py-0.5 rounded-sm bg-black text-white text-[10px] md:text-xs font-bold tracking-widest uppercase">
                    AI Verified
                 </span>
              </div>
          ...
          <div className="mt-6 md:mt-0 flex items-center gap-4">
             ...
             <div className="flex items-center gap-2">
               {onComplete && (
                 <button
                   onClick={onComplete}
                   className="px-6 py-3 bg-gray-100 text-gray-700 font-bold rounded-sm hover:bg-gray-200 transition-colors flex items-center gap-2 text-sm"
                 >
                   완료
                 </button>
               )}
      ...
      <div className="lg:hidden mb-4 flex items-center gap-2 overflow-x-auto pb-2">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-sm text-sm font-semibold whitespace-nowrap transition-all ${activeTab === 'overview' ? 'bg-black text-white' : 'bg-white border border-gray-200 text-gray-600'}`}
        >
          <Layout size={16} /> Overview
        </button>
        <button
          onClick={() => setActiveTab('prd')}
          className={`flex items-center gap-2 px-4 py-2 rounded-sm text-sm font-semibold whitespace-nowrap transition-all ${activeTab === 'prd' ? 'bg-black text-white' : 'bg-white border border-gray-200 text-gray-600'}`}
        >
          <FileText size={16} /> PRD
        </button>
        <button
          onClick={() => setActiveTab('jd')}
          className={`flex items-center gap-2 px-4 py-2 rounded-sm text-sm font-semibold whitespace-nowrap transition-all ${activeTab === 'jd' ? 'bg-black text-white' : 'bg-white border border-gray-200 text-gray-600'}`}
        >
          <Briefcase size={16} /> 구인공고
        </button>
        <div className="flex items-center gap-2 ml-auto">
          <button className="p-2 rounded-sm border border-gray-200 bg-white text-gray-600">
            <Download size={16} />
          </button>
          <button className="p-2 rounded-sm bg-black text-white">
            <Share2 size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-12 gap-4 lg:gap-8 items-start">
        ...
        <div className="hidden lg:block lg:col-span-2">
            <div className="sticky top-6 space-y-1">
                <button
                  onClick={() => setActiveTab('overview')}
                  className={`w-full flex items-center justify-between px-4 py-3 rounded-sm transition-all ${activeTab === 'overview' ? 'bg-white border border-gray-200 shadow-sm text-draft-black' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'}`}
                >
                ...
                <button
                  onClick={() => setActiveTab('prd')}
                  className={`w-full flex items-center justify-between px-4 py-3 rounded-sm transition-all ${activeTab === 'prd' ? 'bg-white border border-gray-200 shadow-sm text-draft-black' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'}`}
                >
                  <div className="flex items-center gap-3">
                    <FileText size={18} />
                ...
                <button
                  onClick={() => setActiveTab('jd')}
                  className={`w-full flex items-center justify-between px-4 py-3 rounded-sm transition-all ${activeTab === 'jd' ? 'bg-white border border-gray-200 shadow-sm text-draft-black' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'}`}
                >
                  <div className="flex items-center gap-3">
                    <Briefcase size={18} />
                ...
                <div className="pt-6 mt-2 border-t border-gray-100 space-y-2 px-1">
                   <button className="w-full py-2 px-3 rounded-sm border border-gray-200 bg-white text-gray-700 text-xs font-bold hover:border-gray-400 hover:shadow-sm flex items-center justify-center gap-2 transition-all">
                      <Download size={14} /> PDF
                   </button>
                   <button className="w-full py-2 px-3 rounded-sm bg-black text-white text-xs font-bold hover:bg-gray-800 hover:shadow-md flex items-center justify-center gap-2 transition-all">
        ...
        <div className="w-full lg:col-span-7 order-1 lg:order-none">
           <div className="relative">
               ...
               {activeTab === 'overview' && (
                   <div className="animate-in fade-in duration-300 space-y-8 bg-white border border-gray-200 rounded-sm p-8 md:p-12 shadow-sm min-h-[800px]">
                       <div>
        ...
        <div className="w-full lg:col-span-3 space-y-3 sm:space-y-4 order-2 lg:order-none">
           ...
           <div className="bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 rounded-sm p-6 shadow-lg overflow-hidden relative">
              ...
              <div className="relative z-10">
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-sm p-3 border border-white/10">
